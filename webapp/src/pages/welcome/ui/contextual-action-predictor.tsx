  ...
  const getPredictedAction = (): PredictedAction => {
    ...
    if (diagnosticCompleted && temporalState === 'past') {
      return {
        ...
        reasoning: 'Tu as déjà fait le diagnostic. Revisite tes résultats ou refais-le pour voir ton évolution.',
      }
    }
    ...
    if (diagnosticStarted && !diagnosticCompleted) {
      return {
        ...
        reasoning: 'Tu as commencé mais pas fini. On t\'attend pour compléter ton évaluation !',
      }
    }
    
    // CAS 3 : État FUTUR + utilisateur impliqué (scroll > 60% OU temps > 30s)
    if (temporalState === 'future' && (scrollDepth > 60 || sessionDuration > 30)) {
      return {
        primary: {
        ...
        reasoning: 'Tu as exploré le mode Action. Tu sembles prêt à passer à l\'étape suivante : évaluer ta situation actuelle.',
      }
    }
    ...
    if (temporalState === 'past' && visitCount >= 2 && visitCount <= 4) {
      return {
        ...
        reasoning: 'Tu reviens pour comparer. Les chiffres t\'intéressent ? Obtiens un rapport personnalisé avec le diagnostic.',
      }
    }
    ...
    if (temporalState === 'present' && !isReturningUser) {
      return {
        ...
        reasoning: 'Première visite ? Prends le temps d\'explorer. Le diagnostic viendra naturellement après.',
      }
    }
    ...
    if (hasReadDeeply) {
      return {
        ...
        reasoning: 'Tu as pris le temps de tout lire (bravo !). Tu es prêt pour un diagnostic personnalisé.',
      }
    }
    ...
    return {
      ...
      reasoning: 'Commence par le diagnostic pour obtenir un plan d\'action personnalisé.',
    }
  }
              ...
              >
                ...
                >
                  ...
                  <div className="flex flex-col items-start">
                    ...
                    <span className="text-xs opacity-90">{predicted.primary.sublabel}</span>
                  </div>
                ...
                >
                  ...
                  {sessionDuration > 30 && (
                    <div className="flex items-center gap-2">
                      ...
                      <span>{Math.floor(sessionDuration / 60)}min d'exploration</span>
                    </div>
                  )}
                  {scrollDepth > 70 && (
                    <div className="flex items-center gap-2">
