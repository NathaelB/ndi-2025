...
const SESSION_STORAGE_KEY = 'ndi-session-data'
...
function getDefaultTemporalState(history: VisitHistory): TemporalState {
  ...
  if (history.diagnosticStarted && !history.diagnosticCompleted) return 'future'
...
export function useTemporalNavigation() {
  ...
  const updateScrollDepth = (depth: number) => {
    setState(prev => {
      const hasReadDeeply = depth > 60 && (Date.now() - prev.sessionStart) > 20000
      return {
        ...prev,
...
export function isShoppingMode(state: TemporalNavigationState): boolean {
  return state.isReturningUser && !state.history.diagnosticStarted && state.history.count >= 2
}
...
export function isReadyToAct(state: TemporalNavigationState): boolean {
  return state.hasReadDeeply || state.history.count >= 3 || state.scrollDepth > 75
}
